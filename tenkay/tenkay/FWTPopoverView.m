...
- (void)layoutSubviews
{
    ...
    //
    if (!self.textLabel.superview) [self addSubview:self.textLabel];
...
#pragma mark - Public
- (void)presentFromRect:(CGRect)rect inView:(UIView *)view withLabel:(UILabel *)thisLabel permittedArrowDirection:(FWTPopoverArrowDirection)arrowDirection animated:(BOOL)animated
{
    //
    ...
    //
    self.textLabel = thisLabel;
    CGSize maximumSize = CGSizeMake(300, 9999);
    CGSize textStringSize = [self.textLabel.text sizeWithFont:[self.textLabel font]
                                            constrainedToSize:maximumSize
                                                lineBreakMode:self.textLabel.lineBreakMode];
    
    rect = CGRectMake(rect.origin.x, rect.origin.y, textStringSize.width, textStringSize.height);
    self.textLabel.frame = rect;
    
    //TODO: is safe to do in this way??
    CGSize currentSize = CGSizeMake(self.textLabel.frame.size.width, self.textLabel.frame.size.height); //self.contentSize;
    ...
    rect = CGRectMake(rect.origin.x, rect.origin.y, self.textLabel.frame.size.width, self.textLabel.frame.size.height);
